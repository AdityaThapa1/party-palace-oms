...
const MealPlanCard = ({ mealType, selectedPlan = { plan: 'None' }, onPlanChange }) => {
    ...
    return (
        <div className="border bg-gray-50 rounded-lg p-4 shadow-sm h-full flex flex-col">
            ...
            <div className="mt-2">
                <label className="text-xs text-gray-600">Choose a Plan</label>
                <select 
                    name="plan" 
                    value={selectedPlan.plan}
                    onChange={(e) => onPlanChange(mealType, e.target.name, e.target.value)}
                    className="mt-1 block w-full bg-white pl-3 pr-8 py-2 text-sm border-gray-300 rounded-md shadow-sm"
                >
            ...
            {items.length > 0 && (
                <div className="mt-4 border-t pt-3 flex-grow">
                    ...
                    <ul className="space-y-1">
                        {items.map(item => <li key={item} className="text-xs text-gray-600 ml-4 list-disc">{item}</li>)}
                    </ul>
                </div>
            )}
        </div>
